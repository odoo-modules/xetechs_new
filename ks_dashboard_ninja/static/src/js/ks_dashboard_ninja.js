            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (items[i].ks_dashboard_item_type === 'ks_tile') {
                    ...
                    if (items[i].id in self.gridstackConfig) {
                        self.grid.addWidget($(item_view), self.gridstackConfig[items[i].id].x, self.gridstackConfig[items[i].id].y, self.gridstackConfig[items[i].id].width, self.gridstackConfig[items[i].id].height, false, 8, null, 2, 2, items[i].id);
                    } else {
                        self.grid.addWidget($(item_view), 0, 0, 8, 2, true, 8, null, 2, 2, items[i].id);
                    }
...
//            var groupBy = item.ks_chart_groupby_type==='relational_type'?item.ks_chart_relation_groupby:item.ks_chart_date_groupby;
