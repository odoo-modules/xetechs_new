    ...
    def pause_task(self):
        ...
        if not self.task_pause:
            if self.task_pause_last_time:
                hours = self.task_run_sum + ((datetime.now() - self.task_pause_last_time).total_seconds() / 3600.0)
            else:
                hours = (datetime.now() - self.task_run_time).total_seconds() / 3600.0
...
class TaskStopDescription(models.TransientModel):
    ...
    def action_stop_task(self):
        ...
        datetime_run_time = task.task_run_time
        project = task.project_id
        if task.task_run_sum:
            if task.task_pause:
                unit_amount = task.task_run_sum
            else:
                unit_amount = task.task_run_sum + ((datetime.now() - task.task_pause_last_time).total_seconds() / 3600.0)
        else:
            unit_amount = (datetime.now() - task.task_run_time).total_seconds() / 3600.0
        ...
        final_stage = project.type_ids.filtered(lambda r: r.final_stage)
        if final_stage:
            task.update({'stage_id': final_stage[0].id})
        self.env['bus.bus'].sendone('need_refresh_tasks', {'model': 'project.task', 'uid': self.env.user.id})
...
class ResConfigSettings(models.TransientModel):
    ...
    project_fast_timesheet = fields.Many2one('project.project', string="Project for fast timesheet", default=1)
