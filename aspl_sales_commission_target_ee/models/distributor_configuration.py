...
class DistributorConfiguration(models.TransientModel):
    ...
    def check_product_commission(self):
        if self.product_commission and self.product_compute_price_type == 'per' and (self.product_commission < 0.0 or self.product_commission > 100):
    ...
    def check_product_categ_commission(self):
        if self.product_categ_commission and self.product_categ_compute_price_type == 'per' and (self.product_categ_commission < 0.0 or self.product_categ_commission > 100):
            raise Warning(_('Commission value for Percentage type must be between 0 to 100.'))

    @api.constrains('customer_commission', 'customer_compute_price_type')
    def check_customers_commission(self):
        if self.customer_commission and self.customer_compute_price_type == 'per' and (self.customer_commission < 0.0 or self.customer_commission > 100):
            raise Warning(_('Commission value for Percentage type must be between 0 to 100.'))

    @api.constrains('team_commission', 'team_compute_price_type')
    def check_customer_commission(self):
        if self.team_commission and self.team_compute_price_type == 'per' and (self.team_commission < 0.0 or self.team_commission > 100):
            raise Warning(_('Commission value for Percentage type must be between 0 to 100.'))

    @api.multi
    def apply_config(self):
        dist_ids = self.distributor_ids if not self.distributor_all else self.env['res.partner'].search([('is_distributor', '=', True)])
        ...
        if dist_ids:
            if self.to_product:
                for product in product_ids:
                    for distributor in dist_ids:
                        product.write({'product_dist_comm_ids': [(0, 0, {'partner_id': distributor.id,
            ...
            if self.to_product_categ:
                for category in category_ids:
                    for distributor in dist_ids:
                        category.write({'prod_categ_dist_comm_ids': [(0, 0, {'partner_id': distributor.id,
            ...
            if self.to_sales_team:
                for team in team_ids:
                    for distributor in dist_ids:
                        team.write({'sale_team_dist_comm_ids': [(0, 0, {'partner_id': distributor.id,
            ...
            raise Warning(_('Please select Distributors to generate commission calculation data.'))
