...
class CrmTeam(models.Model):
    ...
    def _check_commission_values(self):
        ...
        if self.sale_team_dist_comm_ids.filtered(lambda line: line.compute_price_type == 'per' and line.commission > 100 or line.commission < 0.0):
            raise Warning(_('Commission value for Percentage type must be between 0 to 100.'))
        if self.sale_team_cons_comm_ids.filtered(lambda line: line.compute_price_type == 'per' and line.commission > 100 or line.commission < 0.0):
            raise Warning(_('Commission value for Percentage type must be between 0 to 100.'))

    sale_team_comm_ids = fields.One2many('sales.team.commission', 'sale_team_id', string="Sales Person Commission")
    sale_team_dist_comm_ids = fields.One2many('sales.team.distributor.commission', 'sale_team_id', string="Distributor Commission")
    sale_team_cons_comm_ids = fields.One2many('sales.team.consultant.commission', 'sale_team_id', string="Consultant Commission")
